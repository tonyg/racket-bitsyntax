...
(let* ((m (lambda (input)
            (bit-string-case input
              ([ (v :: binary bytes 6) ] (list 'six (bit-string->bytes v)))
              ([ (v :: binary bytes 4) ] (list 'four (bit-string->bytes v)))
              (else 'no))))
       (n (lambda (input)
            (bit-string-case input
              ...
              ([ (v :: binary bytes 4) ] (list 'four (bit-string->bytes v)))
              (else 'no)))))
  (check-equal? (m #"abcdefgh") 'no)
  (check-equal? (m #"abcdef") (list 'six #"abcdef"))
  (check-equal? (m #"abcd") (list 'four #"abcd"))
  (check-equal? (m #"ab") 'no)
  (check-equal? (n #"abcdefgh") 'no)
  (check-equal? (n #"abcdef") (list 'six #"abcdef"))
  (check-equal? (n #"abcd") 'short)
  (check-equal? (n #"ab") 'short))
...
(let* ((m (lambda (input)
  ...
  (check-equal? (m #"abcdefgh") 'no)
  (check-equal? (m #"abcd") (list 'four #"abcd"))
  (check-equal? (m #"abcde") 'no)
  (check-equal? (m #"abcdef") (list 'six #"abcdef"))
  (check-equal? (m #"ab") 'no)
  (check-equal? (n #"abcdefgh") 'no)
  (check-equal? (n #"abcd") (list 'four #"abcd"))
  (check-equal? (n #"abcde") 'short)
  (check-equal? (n #"abcdef") (list 'six #"abcdef"))
  (check-equal? (n #"ab") 'short))
